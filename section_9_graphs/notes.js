...
class FriendGraph{
    ...
    areConnectedBFS(person1,person2){
        ...
        while(toVisitQueue.length){
            ...
            console.log(`Visiting`,currentPerson.name)
    ...
    areConnectedRecursive(person1,person2, seen= new Set([person1])){
        ...
        // for each neighbor we are going to call this same function but we need to keep the seen persisting
        for (let neighbor of person1.adjacent){
            // if we have not seen this neighbor
            if(!seen.has(neighbor)){
                seen.add(neighbor);
